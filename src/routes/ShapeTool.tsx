...
function PrimaryShapeIcon({ shape, color }: { shape: PrimaryShape; color: string }) {
  const scaleFlower = 30 / nn
  const scaleBone = 30 / Xn
  return (
    <svg viewBox="-30 -30 60 60" className="w-full h-full">
      {shape === 'flower' && [0, 45, 90, 135].map((deg) => (
      ...
      {shape === 'bone' && [0, 45, 90, 135].map((deg) => (
        <g key={deg} transform={`rotate(${deg})`}>
      ...
      {shape === 'spike' && [0, 45, 90, 135].map((deg) => (
        <g key={deg} transform={`rotate(${deg})`}>
          <path d={spikePath(24, 8)} fill={color} />
        </g>
      ))}
    </svg>
  )
  ...
  return (
    <svg viewBox="-30 -30 60 60" className="w-full h-full">
      {Array.from({ length: 8 }, (_, i) => {
        const angle = (TAU / 8) * i
        const x = (20 * Math.cos(angle)).toFixed(2)
        const y = (20 * Math.sin(angle)).toFixed(2)
        return (
...
function HighlightShapeMark({ type, size, fill }: { type: HighlightShape; size: number; fill: string }) {
  ...
  // dots
  const r = size * 0.35, off = size * 0.55
  return (
    <>
...
export default function ShapeTool() {
  ...
  const spikeW = Pd * 0.78

  const scaleFlower = go / nn
  const scaleBone = go / Xn

  const primaryItems = useMemo(() => Array.from({ length: count }, (_, j) => {
    const angleDeg = +((m * j + o) * 180 / Math.PI).toFixed(2)
    return { angleDeg, j }
  }), [count, m, o])

  const highlightItems = useMemo(() => Array.from({ length: highlightCount }, (_, j) => {
    const angle = TAU / highlightCount * j + o + m
    const cx = +(da * Math.cos(angle)).toFixed(2)
    const cy = +(da * Math.sin(angle)).toFixed(2)
    const deg = +(angle * 180 / Math.PI).toFixed(2)
    return { cx, cy, deg, j }
  }), [highlightCount, o, m])

  const outerCircles = useMemo(() => Array.from({ length: count }, (_, j) => {
    const angle = TAU / count * j + o
    return { cx: +(fa * Math.cos(angle)).toFixed(2), cy: +(fa * Math.sin(angle)).toFixed(2), j }
  }), [count, o])
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-1">
            {PRIMARY_SHAPES.map((s) => (
              <button
                key={s}
                onClick={() => setPrimaryShape(s)}
                className={`aspect-square p-1 border-2 focus:outline-none transition-colors ${primaryShape === s ? 'border-white' : 'border-gray-700 hover:border-gray-400'}`}
                title={s}
              >
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-1">
            {HIGHLIGHT_SHAPES.map((s) => (
              <button
                key={s}
                onClick={() => setHighlightShape(s)}
                className={`aspect-square p-1 border-2 focus:outline-none transition-colors ${highlightShape === s ? 'border-white' : 'border-gray-700 hover:border-gray-400'}`}
                title={s}
              >
          ...
          <div className="space-y-1">
            {LAYER_NAMES.map((name, i) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-1.5 border text-xs focus:outline-none transition-colors ${layers[i] ? 'border-white bg-white text-black' : 'border-gray-700 bg-transparent text-gray-500 hover:border-gray-400'}`}
              >
